...
load  result_CombDet_test1_224x336_3
% load  result_CombDet_test1_224x336__GaussianOval
BBox1 = BBox;
% load  result_CombDet_test2_224x336_1
% BBox2 = BBox;
% load  result_CombDet_test3_224x336_1
% BBox3 = BBox;
% load  result_CombDet_test4_224x336_1
% BBox4 = BBox;
% load  result_CombDet_test5_224x336_1
% BBox5 = BBox;
% load  result_CombDet_test6_224x336_1
% BBox6 = BBox;

for i=1:size(BBox,1)
%     BBox(i,:) = round(mean([BBox1(i,:);BBox2(i,:);BBox3(i,:);BBox4(i,:);BBox5(i,:);BBox6(i,:)]));
%     BBox(i,:) = round(mean([BBox1(i,:);BBox2(i,:);BBox3(i,:);BBox4(i,:)]));
    BBox(i,:) = BBox1(i,:);
end
...
for i=1:numel(D)
    ...
    dx = max(d, round(0.29*BBox(i,4)));
    dy = max(d, round(0.29*BBox(i,3)));
    
    y1 = dw+BBox(i,1)-dy;
    y2 = dw+BBox(i,1)+BBox(i,3)+dy;
    x1 = dh+BBox(i,2)-dx;
    x2 = dh+BBox(i,2)+BBox(i,4)+dx;
...
% save  BoxXY_CombDet_224x336_1_test1234   XY  ap recall precision results
save  BoxXY_CombDet_224x336_3_test1   XY  ap recall precision results
% save  BoxXY_CombDet_224x336_GaussianOval_test1   XY  ap recall precision results
